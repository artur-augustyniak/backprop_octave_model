...
% TODO brzydki global
	...
	hold on;
      	% MSE PLOT
        plot(1:length(trainRec.perf),trainRec.perf, "Color", "green");
	if (doValidation)
	  % VALIDATION PLOT
	  plot(1:length(trainRec.vperf),trainRec.vperf, "Color","blue");
	endif
        endif
       endif # if !(strcmp(show,"NaN"))
	t = title(sprintf("MSE learning progress %s -  beta %f", label, betaGlobParam));
	set (t, 'interpreter', 'none')
	xlabel("Levenberg Marquardt Epoch");
	ylabel("MSE");
	grid on;
	legend("Training","Validation");

% TODO brzydki global end
